...
export default function Footer() {
  ...
  const tr = lang === "tr";
  const socialLinks = getSocialLinks().filter((link) => link.platform !== "email");
  ...
  return (
    <footer className="border-t border-white/10 bg-slate-950 px-6 py-10 text-white">
      <div className="mx-auto flex max-w-7xl flex-col gap-7 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex gap-4 text-sm font-semibold text-slate-400">{socialLinks.filter((link) => link.platform !== "medium").map((link) => <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="hover:text-white">{link.label}</a>)}</div>
      </div>
